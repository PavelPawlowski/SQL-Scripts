...
GO
/* ****************************************************
sp_CloneRights v0.20 (2015-05-14)
(C) 2010 - 2015 Pavel Pawlowski
...
--Table variable to hold user names
DECLARE @users TABLE (
	UserName sysname
)
...
RAISERROR(N'PRINT ''sp_CloneRights v0.20 (2015-05-14) (C) 2010-2015 Pavel Pawlowski''', 0, 0) WITH NOWAIT;
RAISERROR(N'PRINT ''===============================================================''', 0, 0) WITH NOWAIT;

INSERT INTO @allowedClasses(ClassName, ClassDescription)
VALUES   (N'ROLES_MEMBERSHIP'			, 'Scripts roles membership')
...
--Check if only single user corresponds
SELECT
	@usersCnt = COUNT(1)
FROM sys.database_principals dp
INNER JOIN @users u ON dp.name COLLATE database_default LIKE u.UserName COLLATE database_default

if (@usersCnt = 0 AND @user IS NOT NULL)
BEGIN
	SET @printHelp = 1;
	RAISERROR('@user = ''%s'' does not match to any existing user', 15, 0, @user) WITH NOWAIT;
END
...
BEGIN
	...
	BEGIN
		SET @printHelp = 1;
		RAISERROR('When  @newUser is provided then the @old user murt result to single user', 15, 1) WITH NOWAIT;
	END
END
